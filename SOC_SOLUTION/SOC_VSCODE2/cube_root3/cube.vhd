...
ARCHITECTURE Behavioral OF cube IS
    ...
    TYPE y3_state_type IS (idle, st1, st2, st3, st4);
    SIGNAL y3_state : y3_state_type;

    SUBTYPE int16_t IS INTEGER RANGE -32768 TO 32767;
    SUBTYPE int32_t IS INTEGER RANGE -2147483648 TO 2147483647;

    SIGNAL y3_in : int16_t := 16384;
    SIGNAL y3_out : int16_t := 0;
    SIGNAL y3_temp : int32_t;
    SIGNAL y3_done : STD_LOGIC := '0';
    SIGNAL y3_start : STD_LOGIC := '1';
            ...
            ELSE
                ...
                CASE y3_state IS
                    ...
                    WHEN st1 =>
                        ...
                        y3_state <= st2;
                    WHEN st2 =>
                        y3_temp <= (y3_temp)/2 ** 13;
                        y3_state <= st3;
                    WHEN st3 =>
                        y3_temp <= y3_temp * y3_in;
                        y3_state <= st4;
                    WHEN st4 =>
                        y3_out <= y3_temp / (2 ** 15);
                        y3_state <= idle;
            ...
            ELSE
                ...
                CASE main_state IS
                    ...
                    WHEN sst2 =>
                        y3_start <= '0';
                        IF y3_done = '1' THEN
                            main_temp <= (main_in - y3_out); --int16.11 
                            main_state <= sst3;
                        END IF;
                    WHEN sst3 =>
